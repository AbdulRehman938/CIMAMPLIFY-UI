...
const Header = () => {
    ...
    const handleMemberLogin = () => {
        ...
        if (user) navigate("/");
        else navigate("/authRegister");
    };

    return (
        <div className="bg-surface/70 backdrop-blur w-full flex-wrap min-w-[50%] h-16 fixed top-0 left-0 z-50 flex justify-around whitespace-nowrap items-center border-b border-background font-poppins">
            {/* Logo */}
                ...
                >
                    {navLinks.map((link, index) => (
                        ...
                        >
                            <a
                                href={link.href}
                                className="text-lg font-semibold text-secondary transition-all duration-300 hover:text-primary hover:text-xl"
                            >
                                {link.label}
                            </a>
                    ...
                    {/* Button */}
                    <div className="h-[70%] max-w-[25%] min-w-[25%] bg-primary text-surface font-semibold text-lg flex justify-center items-center rounded-md hover:scale-105 transition-transform duration-200 px-2 hover:drop-shadow-lg">
